...
class TestDllinkProperties:
    ...
    def test_dllink_attach_detach_symmetry_property(self, data1, data2):
        ...
        original_next2 = link2.next
        original_prev2 = link2.prev
    ...
    def test_dllink_chain_property(self, values):
        ...
        if len(links) >= 3:
            ...
            if middle > 0:
                assert links[middle - 1].next is links[middle + 1] if middle + 1 < len(links) else links[0]
            if middle + 1 < len(links):
                assert links[middle + 1].prev is links[middle - 1] if middle > 0 else links[-1]
